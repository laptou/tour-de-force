...
class SelectLevelTile extends PIXI.Container
{
    public level: number;
...
export class SelectLevelScreen extends PIXI.Container implements IScreen
{
    ...
    public async init(app: App): Promise<void>
    {
        ...
        for (let r = 0; r < 3; r++)
        {
            for (let c = 0; c < 5; c++)
            {
                const levelTile = new SelectLevelTile(app, r * 5 + c);
